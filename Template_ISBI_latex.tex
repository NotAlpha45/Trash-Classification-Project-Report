...
\address{$^{1}$Master of Engineering Graduate Program, $^{2}$Electrical and Software Engineering, \\
...
%
\begin{abstract}
With the increasing use of multimodal learning in classification tasks, important questions arise regarding the effectiveness of traditional inference-based models versus retrieval-augmented approaches. In particular, combining image and text modalities introduces challenges in achieving both accuracy and robustness. In this work, we compare supervised multimodal models with retrieval-augmented classification using vector-based heuristics for four-class garbage classification. While supervised fusion models provide strong baseline performance, retrieval-based methods, especially density-aware approaches, show improved robustness and better performance under class imbalance and limited memory conditions.

\end{abstract}
%
\begin{keywords}
Multimodal Lerning, Garbage Classification, Retrieval-Augmented Classification, Vector Heuristics, Deep Learning.
\end{keywords}
%
\section{Introduction}
\label{sec:intro}
Deep learning (DL) models have gained significant attention in multimodal classification due to their ability to learn from heterogeneous data sources such as images and text. By leveraging complementary information across modalities, multimodal approaches often outperform unimodal systems. A common strategy is combine predictions from image and text models using fusion techniques such as weighted logic aggregation, enabling improved accuracy and generalization. However, these supervised approaches rely entirely on learned representations and may face limitations under challenging conditions such as class imbalance or limited training data.

An alternative approach is retrieval-augmented classification, which utilizes embedding-based memory to retrieve similar instance and make predictions based on neighborhood evidence. This allows the model to incorporate additional contextual information beyond the training distribution but introduces challenges in retrieval design and scoring. Despite increasing interest in such methods, a systematic comparison between traditional supervised inference and retrieval-based approaches remains limited. In this work, a two-phase framework is presented to evaluate both strategies for multimodal garbage classification, with experiments conducted under standard settings, class imbalance, and varying memory size to analyze performance, robustness, and generalization.

\begin{figure}[htb]
\centering
\centerline{\includegraphics[width=\linewidth]{Classification.png}}
...
\begin{figure}[htb]
\centering
\centerline{\includegraphics[width=\linewidth]{phase 1 vs 2.png}}
...
\begin{figure}
\centering
\centerline{\includegraphics[width=\linewidth]{Macro F1 scoreing variant.png}}
...
\begin{figure}
\centering
\centerline{\includegraphics[width=\linewidth]{confusion matrix.png}}
...
\begin{figure}
\centering
\centerline{\includegraphics[width=\linewidth]{imbalance.png}}
...
\begin{figure}
\centering
\centerline{\includegraphics[width=\linewidth]{learning curve.png}}
