...
In both the traditional approach and RAC approach, the two modalities - image and text were independently accounted for classification, with their individual classification weights combined for the final prediction like as follows:
  \begin{center}
      $W_{fused} = \alpha W_{text}+(1-\alpha)W_{image}$
  \end{center}    
Where $W_{text}$ and $W_{image}$ are the class scores from text and image models, respectively, and $\alpha \in [0,1]$ controls the contribution of each modality. The hyperparameter $\alpha$ started as 0.5, and was tuned when needed to extract the best classification performance.
