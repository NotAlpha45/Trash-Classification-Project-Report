...
\begin{center}
  ...
  {\rho_m^{\text{local}}(c)}$
\end{center}
\textbf{Kernel Density Estimated Density Normalized Distance Score (KDE DNDS):}
\begin{center}
...
The final RAC prediction is obtained by combining modality-specific scores:
  \begin{center}
      $S_{fused}(c) = \alpha S_{image}(c)+(1-\alpha)S_{text}(c)$,\quad $\hat{y} = \arg\max_{c} S_{fused}(c)$
  \end{center} 
where $S_{image}(c)$ and $S_{text}(c)$ denote image and text retrieval scores.
