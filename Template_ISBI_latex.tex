...
While traditional works with class weights ($W_{image}, W_{text}$) and RAC works with class retrieval scores ($S_{image}(c), S_{text}(c)$), the underlying contribution hyperparameter $\alpha \in [0,1]$ has the same meaning. The hyperparameter $\alpha$ started as 0.5, and was tuned when needed to extract the best classification performance.
